 ...
 *
 * Win32Explorer is an Explorer-like file manager for
 ...
 * -l	Specifies the language for Win32Explorer to use.
 * /?	Causes Win32Explorer to show a small help message and exit.
 *
...
BOOL ProcessCommandLine(TCHAR *pCommandLine)
{
	...
	while((pszCommandLine = GetToken(pszCommandLine,szPath,0)) != NULL)
	{
		...
		else if(lstrcmp(szPath,_T("-remove_as_default")) == 0)
		{
			...
			if(bSuccess)
			{
				MessageBox(NULL,_T("Win32Explorer successfully removed as default file manager."),
					NExplorerplusplus::WINDOW_NAME,MB_OK);
			}
			else
			{
				MessageBox(NULL,_T("Could not remove Win32Explorer as default file manager. Please \
		...
		else if(lstrcmp(szPath,_T("-set_as_default")) == 0)
		{
			...
			if(bSuccess)
			{
				MessageBox(NULL,_T("Win32Explorer successfully set as default file manager."),
					NExplorerplusplus::WINDOW_NAME,MB_OK);
			}
			else
			{
				MessageBox(NULL,_T("Could not set Win32Explorer as default file manager. Please \
ensure you have administrator privileges."),NExplorerplusplus::WINDOW_NAME,MB_ICONWARNING|MB_OK);
			}
		}
		else if(lstrcmp(szPath,NExplorerplusplus::JUMPLIST_TASK_NEWTAB_ARGUMENT) == 0)
		{
			...
			Find the already opened version of Win32Explorer,
			and tell it to open a new tab. */
			HANDLE hMutex;

			hMutex = CreateMutex(NULL,TRUE,_T("Win32Explorer"));
...
LONG WINAPI MyUnhandledExceptionFilter(struct _EXCEPTION_POINTERS *pExceptionInfo)
{
	...
	TCHAR szPath[MAX_PATH];
	TCHAR szAppName[] = _T("Win32Explorer");
	...
	if(hDbgHelp != NULL)
	{
		...
		if(pMiniDumpWriteDump != NULL)
		{
			...
			if(hFile != INVALID_HANDLE_VALUE)
			{
				...
				/*StringCchPrintf(szMsg,SIZEOF_ARRAY(szMsg),_T("Win32Explorer has encountered an error and needs to close. \
	...
	if(GetVersionEx(&VersionInfo) != 0)
	{
		...
		if(VersionInfo.dwMajorVersion < WINDOWS_XP_MAJORVERSION)
		{
			MessageBox(NULL,
				...
				_T("Win32Explorer"),MB_ICONERROR | MB_OK);
