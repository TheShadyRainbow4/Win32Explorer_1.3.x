...
void Explorerplusplus::OnWindowCreate(void)
{
	...
	be set once during execution (i.e. when Win32Explorer
	starts up).
...
void Explorerplusplus::SetLanguageModule(void)
{
	...
	if(g_bForceLanguageLoad)
	{
		...
		StringCchPrintf(szName,SIZEOF_ARRAY(szName),_T("Win32Explorer%2s.dll"),g_szLang);
	...
	else
	{
		...
		PathAppend(szNamePattern,_T("Win32Explorer??.dll"));
